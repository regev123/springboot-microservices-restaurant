...
public class RegisterRequest {
    ...
    @Pattern(regexp = "^[A-Za-z]{3,30}$", message = "First name must be only letters with 3-30 characters")
    private final String firstName;

    @NotBlank(message = "Last name is required")
    @Pattern(regexp = "^[A-Za-z]{3,30}$", message = "Last name must be only letters with 3-30 characters")
